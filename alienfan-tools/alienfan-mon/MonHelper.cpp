...
void CMonProc(LPVOID param) {
	...
	if (!prof)
		return;
	bool modified = false, actionAllowed = src->inControl && !prof->gmode;
	// let's check power...
	if (actionAllowed && acpi->GetPower() != prof->powerStage)
		acpi->SetPower(acpi->powers[prof->powerStage]);
	actionAllowed = actionAllowed && !prof->powerStage;
	// update values:
	// temps..
	...
	for (byte i = 0; i < acpi->fans.size(); i++) {
		...
		int boostCooked = (int)round(src->boostRaw[i] * 100.0 / fan_conf->GetFanScale(i)), curBoost = 0;
		if (actionAllowed && modified) {
			int cBoost;
			for (auto cIter = prof->fanControls.begin(); cIter != prof->fanControls.end(); cIter++)
				if (cIter->first == i) {
					...
					int rawBoost = (int)round((fan_conf->GetFanScale(i) * curBoost) / 100.0);
					if (!src->fanSleep[i] || rawBoost <= 100) {
